...
void clean_up() {
    printf("close server\n");
    if (mq_close(server_queue_ID) == -1) exit_error("Deleting queue", -1);
...
void call_init(struct Message message_buffer) {
    ...
    clients[number_of_client].client_queue_ID = mq_open(message_buffer.message, O_WRONLY);

    if ( clients[number_of_client].client_queue_ID == -1 ){
        printf("Error with init client\n");
        return;
    }

    clients[number_of_client].number_of_friends = 0;
    clients[number_of_client].client_ID = number_of_client;
    clients[number_of_client].client_pid = message_buffer.client_pid;

    response_buffer.client_ID = number_of_client;
    response_buffer.mtype = INIT;

    printf("%d \n", message_buffer.client_queue_ID);

    if (mq_send(clients[number_of_client].client_queue_ID, (char *) &response_buffer, MESSAGE_MAX_SIZE, get_priority(INIT)) == -1 ) exit_error("Server sending init", -1);
    ...
    if (message_buffer == NULL) {
        ...
        for (i = 0; i < MAX_CLIENTS; i++) {
            if (clients[i].client_ID >= 0) {
                ...
                if (mq_send(clients[i].client_queue_ID, (char *) &message, MESSAGE_MAX_SIZE, get_priority(STOP)) == -1 ){
                    printf("Sending echo\n");
                }
        ...
        for (i = 0; i < counter; i++) {
            ...
            if (mq_receive(server_queue_ID, (char *) &message_buffer, MESSAGE_MAX_SIZE, NULL) == -1){
                printf("Error with receiving in server\n");
            }
...
void call_echo(struct Message message_buffer) {
    ...
    if (mq_send(message_buffer.client_queue_ID, (char *) &message, MESSAGE_MAX_SIZE, get_priority(ECHO)) == -1 ){
        printf("Sending echo\n");
    }
...
void call_2all(struct Message message_buffer) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
        ...
        if (i != message_buffer.client_ID && clients[i].client_ID != -1) {
            ...
            if (mq_send(clients[i].client_queue_ID, (char *) &message, MESSAGE_MAX_SIZE, get_priority(ECHO)) == -1 ) {
                printf("Sending echo\n");
            }
...
void call_2friends(struct Message message_buffer) {
    ...
    for (i = 0; i < clients[client_ID].number_of_friends; i++) {
        ...
        if (mq_send(client_queue_ID, (char *) &message, MESSAGE_MAX_SIZE, get_priority(ECHO)) == -1 ) {
            printf("Sending echo\n");
        }
...
void call_2one(struct Message message_buffer) {
    ...
    if (mq_send(clients[message_buffer.client_queue_ID].client_queue_ID,  (char *) &message, MESSAGE_MAX_SIZE, get_priority(ECHO)) == -1 ) {
            printf("Sending echo\n");
    }
...
void call_list(struct Message message_buffer) {
    ...
    if (mq_send(message_buffer.client_queue_ID,  (char *) &message, MESSAGE_MAX_SIZE, get_priority(LIST)) == -1 ) {
        perror("error");
    }
...
void init_server() {
    ...
    server_queue_ID = mq_open("/server", O_RDONLY | O_CREAT | O_EXCL,  0666,  &queue);
